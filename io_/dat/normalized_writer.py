            ...
            with open(dir_original, mode_write ) as original:
                ...
                for ind_sent, (original_sent, normalized_sent) in enumerate(zip(src_ls, pred_ls)):
                    ...
                    for ind, (original_token, normalized_token) in enumerate(zip(original_sent,
                        ...
                        if original_token in SPECIAL_TOKEN_LS and normalized_token in SPECIAL_TOKEN_LS:
                            ind_adjust = 1
                            continue

                        # TODO : when want simultanuous training : assert src_pos src_norm same --> assert pred_pos and pred_norm are same lengh (number of words) ans write
                        if tasks[0] == "normalize":
