    ...
    while True:
        ...
        if not line:
            ...
            if len([word for word in ud.words[sentence_start:] if word.parent is None]) != 1:
                print("OK? ")
                print(UDError("There are multiple roots in a sentence {}".format(ud.words[sentence_start:])))
        ...
        columns = line.split("\t")
        if "-" in columns[0]:
            print("WARNING : (Benjamin added) skipping multiword lines")
            continue
        if len(columns) != 10:
...
def evaluate(gold_ud, system_ud):
    ...
    if gold_ud.characters != system_ud.characters:
        index = 0
        print("WARNING : would raise UD ERROR because of (see next)")
        while index < len(gold_ud.characters) and index < len(system_ud.characters) and \
                (gold_ud.characters[index] == system_ud.characters[index] or "£" == system_ud.characters[index]):
            if "£" in system_ud.characters[index]:
                print("UNK CHARACTER is {} in {} (would fail in real scenario)".format("£",system_ud.characters[index:index + 20]))
            index += 1


        print(UDError(
            "The concatenation of tokens in gold file and in system file differ! on index  indeed : \n" +
            "First 20 differing characters in gold file: '{}' and system file: '{}'. {} ".format(
                
                "".join(gold_ud.characters[-20+index:index + 20]),
                "".join(system_ud.characters[-20+index:index + 20]),str(index), 
            )
        )
        )
